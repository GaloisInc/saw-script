...
parseASTType :: (Functor m, JSS.HasCodebase m) => AST.JavaType -> m SpecJavaType
...
data MethodSpecTranslatorState = MSTS {
       ...
       --, mstsCodebase :: JSS.Codebase
       --, mstsGlobalLetBindings :: Map String (CValue,DagType)
       --, mstsOpMap :: Map String OpDef
       --, mstsOptions :: SSOpts
...
getExprTypeFn :: MethodSpecTranslator (TC.JavaExpr -> Maybe TC.JavaExprDefinedType)
getExprTypeFn = do
  rtm <- gets refTypeMap
  cem <- gets constExprMap
  return $ \e -> case Map.lookup e rtm of
                   Just (RIVArrayConst _ _ tp) -> Just $ TC.JEDTType tp
                   Just (RIVClass cl) -> Just $ TC.JEDTClass (className cl)
                   Just (RIVIntArray l) ->
                     Just $ TC.JEDTType
                          $ SymArray (constantWidth (Wx l)) (SymInt (constantWidth 32))
                   Just (RIVLongArray l) ->
                     Just $ TC.JEDTType
                          $ SymArray (constantWidth (Wx l)) (SymInt (constantWidth 64))
                   Nothing ->
                     case Map.lookup e cem of
                       Nothing -> Nothing
                       Just (_,tp) -> Just (TC.JEDTType tp)
...
checkJSSDagTypeCompat :: Pos -> String -> JSS.Type -> DagType -> MethodSpecTranslator ()
checkJSSDagTypeCompat pos exprName exprType dagType = do
...
resolveDecl (AST.Const pos astJavaExpr astValueExpr) = do
  ...
  checkJSSDagTypeCompat pos (show javaExpr) (TC.getJSSTypeOfJavaExpr javaExpr) tp
...
resolveDecl (AST.MethodLet pos name astExpr) = do
  -- Check var is not already bound locally.
...
resolveDecl (AST.Ensures pos astJavaExpr astValueExpr) = do
  ...
  checkJSSDagTypeCompat pos (show javaExpr) javaExprType valueExprType
  ...
  case JSS.methodReturnType method of
    ...
    Just returnType ->
      ...
      let valueExprType = TC.getTypeOfExpr valueExpr
       in checkJSSDagTypeCompat pos "the return value" returnType valueExprType
...
resolveMethodSpecIR gb pos thisClass mName cmds = do
  ...
  let st = MSTS { mstsPos = pos
                ...
                , specClass = thisClass
                , specMethod = undefined -- Initialized inside execState below.
                , nonRefTypes = Set.empty
                , refTypeMap = Map.empty -- Initialized inside execState below.
...
javaExprValue (jsiThis -> Nothing) (TC.This _) =
  error "internal: javaExprValue given This for static method"
...
evalTypedExpr :: SpecStateInfo -> TC.Expr -> SymbolicMonad Node
evalTypedExpr ssi (TC.Apply op exprs) = do
  applyOp op =<< mapM (evalTypedExpr ssi) exprs
evalTypedExpr _ (TC.Cns c tp) = 
  makeConstant c tp
evalTypedExpr ssi (TC.JavaValue javaExpr _) =
  return $ javaExprNode (ssiJavaStateInfo ssi) javaExpr
evalTypedExpr ssi (TC.Var name tp) = do
...
equivClassMapEntries :: EquivClassMap -> V.Vector (Int,[TC.JavaExpr],SpecJavaRefInitialValue)
