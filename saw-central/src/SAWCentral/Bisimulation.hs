...
buildCompositionSideCondition bc innerBt = do
  ...
  let args = [input, rhsOuterState, lhsOuterState]
  theorem <- io $ scAbstractTerms sc args implication
  ...
  where
    ...
    lambdaOrFail tt =
      case asLambda (ttTerm tt) of
        ...
        Nothing -> do
          sc <- getSharedContext
          tt' <- io $ ppTypedTerm sc tt
          fail $ Text.unpack $ "Error: Expected a lambda term, got " <> tt'
...
extractApp constant term =
  case snd $ go (IntSet.empty, Nothing) term of
    ...
    Nothing -> do
        sc <- getSharedContext
        ppopts <- getPPOpts
        term' <- io $ prettyTerm sc term
        constant' <- io $ prettyTypedTerm sc constant
        fail $ PPS.render ppopts $ PP.vsep [
            "Error: Failed to locate constant in term.",
            "  Constant:" <+> constant',
            "  Term:" <+> term'
         ]
  where
...
buildOutputRelationTheorem bthms bc = do
  ...
  --   srel s1 s2 -> orel (lhs (s1, in)) (rhs (s2, in))
  let args = [input, rhsState, lhsState]
  theorem <- io $ scAbstractTerms sc args implication'
...
buildStateRelationTheorem bc = do
  ...
  -- forall s1 s2 in out1 out2.
  --   orel (s1, out1) (s2, out2) -> srel s1 s2
  let args = [initRhsOutput, initLhsOutput, rhsState, lhsState]
  theorem <- io $ scAbstractTerms sc args implication
  ...
  where
    ...
    typecheckOutputRelation =
      case ttType orel of
        ...
        _ -> do
            sc <- getSharedContext
            orel' <- liftIO $ ppTypedTermType sc (ttType orel)
            fail $ Text.unpack $ "Error: Unexpected output relation type: " <> orel'
    ...
    typecheckStateRelation lhsStateType rhsStateType =
      case ttType srel of
        ...
        _ -> do
            sc <- getSharedContext
            srel' <- liftIO $ ppTypedTermType sc (ttType srel)
            fail $ Text.unpack $ "Error: Unexpected state relation type: " <> srel'
    ...
    typecheckSide side stateType outputType = do
      ...
      case ttType side of
        ...
        _ -> do
          ...
          let stateType' = CryPP.pretty stateType
              input' = "(" <> stateType' <> ", inputType)"
              output' = "(" <> stateType' <> ", outputType)"
          side' <- liftIO $ prettyTypedTermType sc (ttType side)
          fail $ PPS.render ppopts $ PP.vsep [
