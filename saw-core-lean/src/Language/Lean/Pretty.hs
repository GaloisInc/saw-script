...
prettyTerm p e =
  case e of
    ...
    App f args ->
      ...
      -- 'fillSep' on its own packs as much as fits and breaks at arg
      -- boundaries, which is the behaviour we actually want — the
      -- call stays one-line when it fits, breaks naturally otherwise,
      -- and continuation lines align to the App's starting column
      -- without compounding.
