...
dispatchIdentWithArgsWithShape i args
  ...
  -- L-1 (wave-2 release-gate audit, 2026-07-29, HIGH/SILENT). The
  -- arity pattern below USED to be part of this guard
  -- (`, (modArg : _) <- args`), so at zero arguments the whole
  -- alternative failed and fell through to the ordinary dispatch —
  -- which emits, because all seven idents carry non-rejecting
  -- `mapsTo` entries. Zero-argument occurrence is exactly what an
  -- ident in argument, dictionary-field or higher-order position
  -- produces, so the gate could be walked around by never applying
  -- the operation. The eta-adapted result is a well-typed Lean
  -- function universally quantified over `n : Nat` — INCLUDING the
  -- `n = 0` the gate exists to refuse, and every non-literal modulus
  -- it also refuses.
  --
  -- The membership test now stands ALONE and the arity decision moved
  -- inside, so an unapplied occurrence gets a named rejection instead
  -- of silence. That is the general lesson from this class: a
  ...
  = case args of
      [] ->
        Except.throwError (RejectedPrimitive
          ...
          ("an unapplied IntMod operation is rejected: the modulus "
           <> "gate can only inspect a modulus that is present, and "
           <> "an unapplied occurrence would emit a function "
           <> "quantified over EVERY n — including n = 0, where SAW "
           <> "has no coherent semantics (see the applied-form "
           <> "rejection). Apply the operation to a concrete literal "
           <> "modulus >= 1."))
      (modArg : _) -> do
