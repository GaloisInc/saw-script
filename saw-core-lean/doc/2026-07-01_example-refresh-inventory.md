...
| `drivers/cryptol_module_point` | `current-emission` | Full suite failed due stale emission. | Record/tuple rows pass; proof counterpart separate. | Review before golden refresh. |
...
| `drivers/cryptol_module_record_update` | `current-emission` | Full suite failed; proof `point_shift_property` drifted. | `differential/record_update`; proof fixture stale. | Review and refresh only with proof decision. |
...
| `drivers/cryptol_running_sum_verify` | `proof-gap` | Full suite failed. | Generic fix/proof-carrying recurrence surface. | Keep as proof gap; do not restore deleted recurrence helpers. |
...
| `drivers/offline_lean_popcount32` | `stress` | Full suite failed. | BV-heavy popcount proof surface. | Keep stress/proof gap; no native-eval proof shortcuts. |
...
| `proofs/conformance_algebraic` | `legacy-retire` | Lean support proof passed, but not a generated proof-backend example. | Legacy conformance driver. | Keep only as support regression or migrate out of proof corpus. |
| `proofs/conformance_bitvector` | `legacy-retire` | Lean support proof passed, but not a generated proof-backend example. | Legacy conformance driver. | Keep only as support regression or migrate out of proof corpus. |
| `proofs/conformance_bitvector_conversions` | `legacy-retire` | Lean support proof passed, but not a generated proof-backend example. | Legacy conformance driver. | Keep only as support regression or migrate out of proof corpus. |
| `proofs/conformance_boolean` | `legacy-retire` | Lean support proof passed, but not a generated proof-backend example. | Legacy conformance driver. | Keep only as support regression or migrate out of proof corpus. |
| `proofs/conformance_core` | `legacy-retire` | Lean support proof passed, but not a generated proof-backend example. | Legacy conformance driver. | Keep only as support regression or migrate out of proof corpus. |
| `proofs/conformance_error` | `legacy-retire` | Lean support proof passed, but not a generated proof-backend example. | Legacy conformance driver. | Keep only as support regression or migrate out of proof corpus. |
| `proofs/conformance_record` | `legacy-retire` | Lean support proof passed, but not a generated proof-backend example. | Legacy conformance driver. | Keep only as support regression or migrate out of proof corpus. |
| `proofs/conformance_scalar` | `legacy-retire` | Lean support proof passed, but not a generated proof-backend example. | Legacy conformance driver. | Keep only as support regression or migrate out of proof corpus. |
| `proofs/conformance_scalar_extra` | `legacy-retire` | Lean support proof passed, but not a generated proof-backend example. | Legacy conformance driver. | Keep only as support regression or migrate out of proof corpus. |
| `proofs/conformance_stream` | `legacy-retire` | Lean support proof passed, but not a generated proof-backend example. | Legacy conformance driver. | Keep only as support regression or migrate out of proof corpus. |
| `proofs/conformance_string` | `legacy-retire` | Lean support proof passed, but not a generated proof-backend example. | Legacy conformance driver. | Keep only as support regression or migrate out of proof corpus. |
| `proofs/conformance_string_bytes` | `legacy-retire` | Lean support proof passed, but not a generated proof-backend example. | Legacy conformance driver. | Keep only as support regression or migrate out of proof corpus. |
| `proofs/conformance_tuple` | `legacy-retire` | Lean support proof passed, but not a generated proof-backend example. | Legacy conformance driver. | Keep only as support regression or migrate out of proof corpus. |
| `proofs/conformance_vector` | `legacy-retire` | Lean support proof passed, but not a generated proof-backend example. | Legacy conformance driver. | Keep only as support regression or migrate out of proof corpus. |
| `proofs/conformance_vector_zip` | `legacy-retire` | Lean support proof passed, but not a generated proof-backend example. | Legacy conformance driver. | Keep only as support regression or migrate out of proof corpus. |
| `proofs/cookbook` | `legacy-retire` | Pins support-library cookbook patterns, not emitted proof artifacts. | `doc/proof-cookbook.md`. | Keep as support-library doc test or move out of proof-backend examples. |
...
| `proofs/point_shift_property` | `proof-gap` | Current emitted module differs from tracked `.lean.good`. | `drivers/cryptol_module_record_update`. | Review module diff before repair/refresh. |
...
| --- | --- | --- | --- |
| `doc/proof-cookbook.md` | `legacy-retire` | Covered by `proofs/cookbook`, but this is support-library proof guidance, not generated proof-backend replay. | Keep useful content, but do not count as current proof-backend success until examples import generated artifacts. |
...
3. For `point_shift_property`, inspect the producer diff before deciding
   whether it is a small proof repair or a real proof gap. `E3` is now repaired
   and `E4`/`E5` are explicit proof gaps.
4. Do not refresh `conformance_stream`, `sequences.t18`, `implRev4`, stream
   ...
   blocker.
5. Decide whether legacy support-library proof rows under `proofs/conformance_*`
   and `proofs/cookbook` should move to a support-library test bucket so the
   `proofs/` directory only advertises generated proof-backend examples.
