...
-- | A set containing a single half-open interval @[i..j-1]@.
singleton :: (Int, Int) -> IntervalSet
singleton (i, j)
  | i < j = IntervalSet (IntMap.singleton i j)
  | otherwise = empty

-- | Add an interval @[i..j-1]@ to the set.
  ...
  | otherwise =
    let -- m1 contains all intervals strictly below i
        ...
        i' = case IntMap.lookupLE i m of
               Nothing -> i
               Just (u, v) -> if i <= v then u else i
        j' = case IntMap.lookupLE j m of
               Nothing -> j
               Just (_, v) -> max v j
    ...
    -- If i falls inside (or coincides with the upper bound of) an
    -- interval [u, v), set i' = u; else i' = i.
    -- If j falls inside an interval [u, v), let j' = v; else j' = j.
    -- Always insert a new interval [i', j').

-- | Remove an interval @[i..j-1]@ from the set.
  ...
  | otherwise =
    let m1 = IntMap.takeWhileAntitone (< i) m
        ...
        m1' = case IntMap.lookupLT i m1 of
                Nothing -> m1
                Just (u, v) -> if i < v then IntMap.insert u i m1 else m1
        m2' = case IntMap.lookupLE j m of
                Nothing -> m2
                Just (_, v) -> if j < v then IntMap.insert j v m2 else m2
    in IntervalSet (IntMap.union m1' m2')
    -- Any interval [u, v) with v <= i stays unchanged.
    -- Any interval [u, v) with u < i < v is overwritten.
    -- Any interval [u, v) with i <= u <= j is removed.
    -- Any interval [u, v) with j < u stays unchanged.
    -- If i falls inside an interval [u, v), replace it with interval [u, i).
    -- If j falls inside an interval [u, v), insert a new interval [j, v).
...
-- | Create a set from a list of half-open intervals.
fromList :: [(Int, Int)] -> IntervalSet
fromList = foldr insert empty

-- | Convert the set to a list of disjoint half-open intervals in
-- ascending order.
