...
import SAWScript.Options (Options (printOutFn), Verbosity, printOutLn)
...
import Verifier.SAW.Rewriter (Simpset, lhsRewriteRule, listRules, rhsRewriteRule)
import Verifier.SAW.SharedTerm hiding
  ( PPOpts (..),
    defaultPPOpts,
    ppTerm,
    scPrettyTerm,
  )
import qualified Verifier.SAW.SharedTerm as SAWCorePP
  ( PPOpts (..),
    defaultPPOpts,
    ppTerm,
    scPrettyTerm,
...
data PrimitiveLifecycle
  = -- | Currently available in all modes.
    Current
  | -- | Will be removed soon, and available only when
    --                         requested.
    Deprecated
  | -- | Will be made @Current@ soon, but available only by
    --                         request at the moment.
    Experimental
  deriving (Eq, Ord, Show)

-- | TopLevel Read-Only Environment.
data TopLevelRO = TopLevelRO
...
data TopLevelRW = TopLevelRW
  { rwValues :: Map SS.LName Value,
    ...
    -- | Values, generated anywhere, that represent proofs.
    rwProofs :: [Value],
    rwPPOpts :: PPOpts,
...
extendEnv x mt md v rw =
  rw
    { rwValues = M.insert name v (rwValues rw),
...
data JavaSetupState = JavaSetupState
...
data LLVMCrucibleSetupM a = LLVMCrucibleSetupM
...
addTrace str val =
  case val of
    ...
    VLLVMCrucibleSetup (LLVMCrucibleSetupM m) ->
      VLLVMCrucibleSetup $
        LLVMCrucibleSetupM $
