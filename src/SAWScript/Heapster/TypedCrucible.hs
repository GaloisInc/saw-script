...
data TypedStmt ext (rets :: RList CrucibleType) ps_in ps_out where
  ...
  CreateLifetime :: TypedStmt ext (RNil :> LifetimeType)
...
-- | Create a new lifetime @l@ and add @l:lowned(nil)@ to the top of the
-- permission stack
newLifetime :: StmtPermCheckM ext cblocks blocks ret args
               (RNil :> LifetimeType) RNil (ExprVar LifetimeType)
newLifetime =
  let loc =
        mkProgramLoc (functionNameFromText "None")
        (OtherPos "(Generated by permission type-checker)") in
  emitStmt knownRepr loc CreateLifetime >>>= \(_ :>: n) -> greturn n
...
tcEmitLLVMStmt arch ctx loc (LLVM_PopFrame _) =
  ...
  case (maybe_fp, fp_perm) of
    (Just fp, fp_perm@(ValPerm_Conj [Perm_LLVMFrame fperms]))
