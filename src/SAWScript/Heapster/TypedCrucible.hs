...
data TypedLLVMStmt w ret ps_in ps_out where
  ...
  -- | Assert that an LLVM pointer is a pointer, and return 1 as a
  -- 'NatType'. This is the typed version of 'LLVM_PointerBlock' when we know
  -- the input is a pointer, i.e., has a non-zero pointer block value.
  --
  ...
  -- assumes that the behavior of the program we are verifying is not altered in
  -- a meaningful way by mapping the return value of 'LLVM_PointerBlock' to 1
  -- when it is applied to pointers, which is the case for all programs
  -- currently generated by Crucible from LLVM.
  --
  -- Type: @x:ptr((rw,off) |-> p) -o x:ptr((rw,off) |-> p), ret:eq(1)@
  AssertLLVMPtrNat :: (1 <= w2, KnownNat w2) =>
                      TypedReg (LLVMPointerType w2) -> LLVMFieldPerm w2 ->
                      TypedLLVMStmt w NatType
                      (RNil :> LLVMPointerType w2)
                      (RNil :> LLVMPointerType w2 :> NatType)

  -- | Similar to 'AssertLLVMPtrNat' but for array permissions
  --
  -- Type: @x:array(ap) -o x:array(ap), ret:eq(1)@
  AssertLLVMArrayNat :: (1 <= w2, KnownNat w2) =>
                        TypedReg (LLVMPointerType w2) -> LLVMArrayPerm w2 ->
                        TypedLLVMStmt w NatType
                        (RNil :> LLVMPointerType w2)
                        (RNil :> LLVMPointerType w2 :> NatType)
  ...
  -- | Assert that an LLVM pointer is a pointer, and return 0 as a
  -- bitvector. This is the typed version of 'LLVM_PointerOffset' when we know
  -- the input is a pointer, i.e., has a non-zero pointer block value.
  --
  -- NOTE: Just as with 'AssertLLVMPtrNat', because our SAW translation does not
  -- include any computational content for pointer blocks and offsets, we cannot
  -- represent the actual runtime value of the offset of a pointer. We thus
  -- return 0 as a dummy value. This implicitly assumes that the behavior of the
  -- program we are verifying is not altered in a meaningful way by mapping the
  -- return value of 'LLVM_PointerOffset' to 0 when it is applied to pointers,
  -- which is the case for all programs currently generated by Crucible from
  -- LLVM.
  --
  -- Type: @x:ptr((rw,off) |-> p) -o x:ptr((rw,off) |-> p), ret:eq(0)@
  AssertLLVMPtrBV :: (1 <= w2, KnownNat w2) =>
                     TypedReg (LLVMPointerType w2) -> LLVMFieldPerm w2 ->
                     TypedLLVMStmt w (BVType w2)
                     (RNil :> LLVMPointerType w2)
                     (RNil :> LLVMPointerType w2 :> BVType w2)

  -- | Similar to 'AssertLLVMPtrBV' but for array permissions
  --
  -- Type: @x:array(ap) -o x:array(ap), ret:eq(0)@
  AssertLLVMArrayBV :: (1 <= w2, KnownNat w2) =>
                        TypedReg (LLVMPointerType w2) -> LLVMArrayPerm w2 ->
                        TypedLLVMStmt w (BVType w2)
                        (RNil :> LLVMPointerType w2)
                        (RNil :> LLVMPointerType w2 :> BVType w2)
  ...
  -- Type:
  -- > ps, x:ptr((rw,0) |-> p), cur_ps
  -- > -o ps, x:ptr((rw,0) |-> eq(y)), cur_ps
  TypedLLVMStore :: (1 <= w, KnownNat w) => TypedReg (LLVMPointerType w) ->
                    LLVMFieldPerm w -> TypedReg (LLVMPointerType w) ->
                    DistPerms ps -> LifetimeCurrentPerms ps_l ->
...
typedLLVMStmtIn (AssertLLVMPtrNat (TypedReg x) fp) =
  distPerms1 x (ValPerm_Conj1 $ Perm_LLVMField fp)
typedLLVMStmtIn (AssertLLVMArrayNat (TypedReg x) ap) =
  distPerms1 x (ValPerm_Conj1 $ Perm_LLVMArray ap)
typedLLVMStmtIn (DestructLLVMWord (TypedReg x) e) =
  distPerms1 x (ValPerm_Eq $ PExpr_LLVMWord e)
typedLLVMStmtIn (AssertLLVMPtrBV (TypedReg x) fp) =
  distPerms1 x (ValPerm_Conj1 $ Perm_LLVMField fp)
typedLLVMStmtIn (AssertLLVMArrayBV (TypedReg x) ap) =
  distPerms1 x (ValPerm_Conj1 $ Perm_LLVMArray ap)
...
typedLLVMStmtOut (AssertLLVMPtrNat (TypedReg x) fp) ret =
  distPerms2 x (ValPerm_Conj1 $ Perm_LLVMField fp)
  ret (ValPerm_Eq $ PExpr_Nat 1)
typedLLVMStmtOut (AssertLLVMArrayNat (TypedReg x) ap) ret =
  distPerms2 x (ValPerm_Conj1 $ Perm_LLVMArray ap)
  ret (ValPerm_Eq $ PExpr_Nat 1)
typedLLVMStmtOut (DestructLLVMWord (TypedReg x) e) ret =
  distPerms1 ret (ValPerm_Eq e)
typedLLVMStmtOut (AssertLLVMPtrBV (TypedReg x) fp) ret =
  distPerms2 x (ValPerm_Conj1 $ Perm_LLVMField fp)
  ret (ValPerm_Eq $ PExpr_BV [] 0)
typedLLVMStmtOut (AssertLLVMArrayBV (TypedReg x) ap) ret =
  distPerms2 x (ValPerm_Conj1 $ Perm_LLVMArray ap)
  ret (ValPerm_Eq $ PExpr_BV [] 0)
...
typedLLVMStmtOut (TypedLLVMStore (TypedReg x) fp (TypedReg y) ps cur_ps) _ =
  ...
  (DistPermsCons ps x (ValPerm_Conj1 $ Perm_LLVMField $
                       fp { llvmFieldContents = ValPerm_Eq $ PExpr_Var y }))
  (lifetimeCurrentPermsPerms cur_ps)
...
-- Type-check the LLVM pointer destructor that gets the block, which 
  ...
  case eith of
    ...
    Right ps
      ...
      , Perm_LLVMField fp <- ps!!i ->
        ...
        emitLLVMStmt knownRepr loc (AssertLLVMPtrNat tptr_reg fp) >>>= \ret ->
        stmtRecombinePerms >>>
        greturn (addCtxName ctx ret)
    Right ps
      ...
      , Perm_LLVMArray ap <- ps!!i ->
        ...
        (implExtractConjM x ps i >>>
         recombinePerm x (ValPerm_Conj (deleteNth i ps))) >>>
        emitLLVMStmt knownRepr loc (AssertLLVMArrayNat tptr_reg ap) >>>= \ret ->
        stmtRecombinePerms >>>
        greturn (addCtxName ctx ret)
    Right ps ->
      stmtFailM (\i -> "LLVM_PointerBlock: LLVM value without pointer or word permissions: "
                       <> permPretty i tptr_reg
                       </> string "permissions: "
                       <> permPretty i (ValPerm_Conj ps))
...
tcEmitLLVMSetExpr arch ctx loc (LLVM_PointerOffset w ptr_reg) =
  ...
  case eith of
    Left e ->
      emitLLVMStmt knownRepr loc (DestructLLVMWord tptr_reg e) >>>= \ret ->
      stmtRecombinePerms >>>
      greturn (addCtxName ctx ret)
    Right ps
      | Just i <- findIndex isLLVMFieldPerm ps
      , Perm_LLVMField fp <- ps!!i ->
        ...
        (implExtractConjM x ps i >>>
         recombinePerm x (ValPerm_Conj (deleteNth i ps))) >>>
        emitLLVMStmt knownRepr loc (AssertLLVMPtrBV tptr_reg fp) >>>= \ret ->
        stmtRecombinePerms >>>
        greturn (addCtxName ctx ret)
    Right ps
      | Just i <- findIndex isLLVMArrayPerm ps
      , Perm_LLVMArray ap <- ps!!i ->
        ...
        (implExtractConjM x ps i >>>
         recombinePerm x (ValPerm_Conj (deleteNth i ps))) >>>
        emitLLVMStmt knownRepr loc (AssertLLVMArrayBV tptr_reg ap) >>>= \ret ->
        stmtRecombinePerms >>>
        greturn (addCtxName ctx ret)
    Right ps ->
      stmtFailM (\i -> "LLVM_PointerOffset: LLVM value without pointer or word permissions: "
                       <> permPretty i tptr_reg
                       </> string "permissions: "
                       <> permPretty i (ValPerm_Conj ps))
...
emitTypedLLVMStore ::
  ...
  TypedReg (LLVMPointerType (ArchWidth arch)) -> DistPerms ps ->
  StmtPermCheckM (LLVM arch) cblocks blocks ret args
  (ps :> LLVMPointerType w)
  (ps :> LLVMPointerType w)
  (Name UnitType)
emitTypedLLVMStore _ loc treg_ptr fp treg_val ps =
  withLifetimeCurrentPerms (llvmFieldLifetime fp) $ \cur_perms ->
  emitLLVMStmt knownRepr loc (TypedLLVMStore treg_ptr fp treg_val ps cur_perms)
