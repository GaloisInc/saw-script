...
<!--
Counterexamples are any value for `y`, and a value for `x` that's
greater than or equal to `y`.
For `y` == 0, that's all 256 possible values of `x`; for `y` == 255,
there's one, namely 255.
Thus the number of counterexamples is the sum of 1..256, which is
256 * (256+1)/2, or 128 * 257, which is 32896.
There is no need to explain this (or include this text), but we'd best
have the correct number so as not to look like fools :-)
-->
