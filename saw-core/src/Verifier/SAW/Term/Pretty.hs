...
ppFlatTermF prec tf =
  case tf of
    ...
    ExtCns cns -> do
      bn <- ppBestName (ecName cns)
      pure $ annotate ExtCnsStyle (bn <> "(" <> pretty (ecVarIndex cns) <> ")")
...
ppTermF _ (Constant ec _) = do
  bn <- ppBestName (ecName ec)
  pure $ annotate ConstantStyle (bn <> "(" <> pretty (ecVarIndex ec) <> ")")
