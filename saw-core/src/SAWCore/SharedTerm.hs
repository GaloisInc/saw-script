...
-- primitive fpFromBV : (w : Nat) (e : Nat) -> (p : Nat) -> RoundingMode -> Vec w Bool -> Float e p;
scFpFromBV :: SharedContext -> Term -> Term -> Term -> Term -> Term -> IO Term
scFpFromBV sc w e p m v = scGlobalApply sc "Prelude.fpFromBV" [w, e, p, m, v]

-- primitive fpFromSBV : (w : Nat) (e : Nat) -> (p : Nat) -> RoundingMode -> Vec w Bool -> Float e p;
scFpFromSBV :: SharedContext -> Term -> Term -> Term -> Term -> Term -> IO Term
scFpFromSBV sc w e p m v = scGlobalApply sc "Prelude.fpFromSBV" [w, e, p, m, v]
...
-- primitive fpToBV : (e : Nat) -> (p : Nat) (w : Nat) -> RoundingMode -> Float e p -> Vec w Bool;
scFpToBV :: SharedContext -> Term -> Term -> Term -> Term -> Term -> IO Term
scFpToBV sc e p w m f = scGlobalApply sc "Prelude.fpToBV" [e, p, w, m, f]

-- primitive fpToSBV : (e : Nat) -> (p : Nat) (w : Nat) -> RoundingMode -> Float e p -> Vec w Bool;
scFpToSBV :: SharedContext -> Term -> Term -> Term -> Term -> Term -> IO Term
scFpToSBV sc e p w m f = scGlobalApply sc "Prelude.fpToSBV" [e, p, w, m, f]
