...
import SAWSupport.IntervalSet (IntervalSet)
import qualified SAWSupport.IntervalSet as IntervalSet
...
data SharedContext = SharedContext
  ...
  , scValidTerms     :: IORef IntervalSet
  }
...
scEnsureValidTerm sc t =
  do s <- readIORef (scValidTerms sc)
     unless (IntervalSet.member (termIndex t) s) $
       fail $ unlines $
