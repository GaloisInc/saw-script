...
Import :: { Import }
 : mbSub nameParams mbAs mbImportSpec   {% buildImport $1 $2 $3 $4 }

mbSub :: { Bool }
 : {- empty -}                          { False }
 | 'submodule'                          { True }

nameParams :: { ((Either FilePath P.ModName, Pos), Maybe (Pos, Text)) }
 : MName                                { ($1, Nothing) }
 | '`' MName                            { ($2, Just (tokPos $1, "_")) }
 | MName '{' name '}'                   { ($1, Just (tokPos $3, tokStr $3)) }
...
buildImport ::
    Bool ->
    ((Either FilePath P.ModName, Pos), Maybe (Pos, Text)) ->
    (Maybe P.ModName, Pos) ->
    (Maybe P.ImportSpec, Pos) ->
    Either ParseError Import
buildImport issub ((modName, namePos), params) (mbAsName, asPos) (mbSpec, specPos) =
  let imp isBacktick = Import {
              iIsSubmodule = issub,
              iModule = modName,
              iIsBacktick = isBacktick,
              iAs = mbAsName,
              iSpec = mbSpec,
  ...
  in
  case params of
      Nothing -> Right $ imp False
      Just (_, "_") -> Right $ imp True
      Just (pos, _) -> Left $ UnsupportedModuleParameters pos
