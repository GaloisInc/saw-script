...
primitives = Map.fromList $
 ...
 , prim "enable_crucible_assert_then_assume" "TopLevel ()"
  ...
  , prim "offline_lean" "String -> ProofScript ()"
    ...
    , "For SAW-side discharge, see 'offline_lean_replay'."
    , ""
  ...
  , prim "offline_lean_replay" "String -> ProofScript ()"
    ...
    Current
    [ "Check a user-written Lean proof of the current goal and ADMIT"
    , "the goal if it passes. The argument is a directory containing:"
    , ""
    , "  proof.lean      (required) the discharge; must define a named"
    , "                  theorem 'goal_closed' of the goal's type."
    , "  completed.lean  (optional) a completed copy of the generated"
    , "                  outline, for goals whose emitted definitions"
    , "                  carry side-condition placeholders."
    , ""
    , "SAW re-emits the goal FRESH in-process and treats that as the"
    , "authority: your copy of the emitted artifact is never trusted, so"
    , "a proof written against a stale or edited goal fails loudly. The"
    , "proof is then checked by the factored trust kernel — the emitted"
    , "goal must compile, the proof must elaborate with no 'sorry', it"
    , "must actually prove the emitted goal, and every named theorem is"
    , "audited with '#print axioms' against a fixed allowlist (propext,"
    , "Classical.choice, Quot.sound, and the two documented Vec/BitVec"
    , "bridge axioms). Any failure is a ProofScript failure naming the"
    , "check; there are no silent outcomes."
    , ""
    , "Runs STRICT-tier only: proofs depending on native-evaluation"
    , "axioms (e.g. 'bv_decide', 'native_decide') are REJECTED here."
    , "The 'native-eval' trust tier is a conformance-suite construct"
    , "and is never honored at runtime admission."
    , ""
    , "On success the goal is recorded with LeanReplayEvidence (the"
    , "pinned toolchain, goal/proof fingerprints, and the audited axiom"
    , "list). Note this is a one-shot trust token: it documents the"
    , "kernel check, it cannot re-verify it."
    , ""
    , "Lean assets resolve from the installed data-files; set"
    , "SAW_LEAN_ROOT to a saw-script checkout to override (dev/CI)."
    ]
