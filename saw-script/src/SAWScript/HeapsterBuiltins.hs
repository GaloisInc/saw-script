...
import qualified Data.Text as T  -- XXX remove uses of this (too many for just now)
...
checkTypeAgreesWithDesc :: SharedContext -> PermEnv -> Text -> Ident ->
...
-- `Nothing`.
failOnNothing :: Fail.MonadFail m => Text -> Maybe a -> m a
failOnNothing err_str Nothing = Fail.fail $ Text.unpack err_str
...
lookupFunctionDecl :: LLVMModule arch -> Text -> Maybe L.Declare
lookupFunctionDecl lm nm =
  find (((fromString $ Text.unpack nm) ==) . L.decName) $ L.modDeclares $ modAST lm

-- | Look up the 'L.Define' for a symbol defined in an 'LLVMModule'
lookupFunctionDef :: LLVMModule arch -> Text -> Maybe L.Define
lookupFunctionDef lm nm =
  find (((fromString $ Text.unpack nm) ==) . L.defName) $ L.modDefines $ modAST lm

-- | Lookup a the singnature for a symbol in an 'LLVMModule'. This
--   will find a signaure for either an external symbol, or for
--   a defined symbol
lookupFunctionDeclOrDef :: LLVMModule arch -> Text -> Maybe L.Declare
...
lookupFunctionCFG :: LLVMModule arch -> Text -> IO (Maybe (AnyCFG Lang.Crucible.LLVM.Extension.LLVM))
lookupFunctionCFG lm nm =
  getTranslatedCFG (modTrans lm) (fromString $ Text.unpack nm) >>= \case
    Nothing -> return Nothing
    Just (_,cfg,_warns) -> return (Just cfg)

-- | Look up the argument and return types of a named function
lookupFunctionType :: LLVMModule arch -> Text ->
...
lookupFunctionType (lm :: LLVMModule arch) nm =
  case lookupFunctionDeclOrDef lm nm of
    ...
    Nothing ->
      fail $ Text.unpack $ "Could not find symbol: " <> nm

-- | Look for the LLVM module in a 'HeapsterEnv' where a symbol is defined
lookupModDefiningSym :: HeapsterEnv -> Text -> Maybe (Some LLVMModule)
...
lookupModContainingSym :: HeapsterEnv -> Text -> Maybe (Some LLVMModule)
...
lookupLLVMSymbolModAndCFG :: HeapsterEnv -> Text -> IO (Maybe (Some ModuleAndCFG))
...
heapster_init_env :: Text -> FilePath -> TopLevel HeapsterEnv
heapster_init_env mod_str llvm_filename =
  heapster_init_env_gen noDebugLevel mod_str llvm_filename

heapster_init_env_debug :: Text -> FilePath -> TopLevel HeapsterEnv
heapster_init_env_debug mod_str llvm_filename =
  heapster_init_env_gen traceDebugLevel mod_str llvm_filename

heapster_init_env_gen :: DebugLevel -> Text -> FilePath -> TopLevel HeapsterEnv
heapster_init_env_gen dlevel mod_str llvm_filename =
...
heapster_init_env_from_file :: FilePath -> FilePath -> TopLevel HeapsterEnv
heapster_init_env_from_file mod_filename llvm_filename =
  heapster_init_env_for_files_gen noDebugLevel mod_filename [llvm_filename]

heapster_init_env_from_file_debug :: FilePath -> FilePath -> TopLevel HeapsterEnv
heapster_init_env_from_file_debug mod_filename llvm_filename =
  heapster_init_env_for_files_gen traceDebugLevel mod_filename [llvm_filename]

heapster_init_env_for_files_gen :: DebugLevel ->
                                   FilePath -> [FilePath] ->
                                   TopLevel HeapsterEnv
heapster_init_env_for_files_gen dlevel mod_filename llvm_filenames =
...
heapster_init_env_for_files :: FilePath -> [FilePath] -> TopLevel HeapsterEnv
heapster_init_env_for_files mod_filename llvm_filenames =
  heapster_init_env_for_files_gen noDebugLevel mod_filename llvm_filenames

heapster_init_env_for_files_debug :: FilePath -> [FilePath] -> TopLevel HeapsterEnv
heapster_init_env_for_files_debug mod_filename llvm_filenames =
  heapster_init_env_for_files_gen traceDebugLevel mod_filename llvm_filenames

-- | Look up the CFG associated with a symbol name in a Heapster environment
-- XXX why does this take the BuiltinContext and Options if it doesn't use them?
heapster_get_cfg :: BuiltinContext -> Options -> HeapsterEnv ->
                    Text -> TopLevel SAW_CFG
heapster_get_cfg _ _ henv nm =
  case lookupModDefiningSym henv nm of
    Just (Some lm) -> llvm_cfg (Some lm) nm
    Nothing -> fail $ Text.unpack $ "Could not find CFG for symbol: " <> nm
...
heapster_define_opaque_perm _bic _opts henv nm args_str tp_str term_str d_str =
  do env <- liftIO $ readIORef $ heapsterEnvPermEnvRef henv
     Some args <- parseCtxString "argument types" env (Text.unpack args_str)
     Some tp_perm <- parseTypeString "permission type" env (Text.unpack tp_str)
     sc <- getSharedContext
     term_tp <- liftIO $ translateExprTypeFunType sc env args
     term_ident <- parseAndInsDef henv (Text.unpack nm) term_tp (Text.unpack term_str)
     d_tp <- tpDescTypeM sc
     d_ident <- parseAndInsDef henv (Text.unpack nm ++ "__desc") d_tp (Text.unpack d_str)
     liftIO $ checkTypeAgreesWithDesc sc env nm term_ident args d_ident
     let env' = permEnvAddOpaquePerm env (Text.unpack nm) args tp_perm term_ident d_ident
...
heapster_define_recursive_perm _bic _opts henv nm args_str tp_str p_str =
  do env <- liftIO $ readIORef $ heapsterEnvPermEnvRef henv
     ...
     Some args_ctx <- parseParsedCtxString "argument types" env (Text.unpack args_str)
     Some tp <- parseTypeString "permission type" env (Text.unpack tp_str)
     let args = parsedCtxCtx args_ctx
         args_p = CruCtxCons args (ValuePermRepr tp)
     mb_p <- parsePermInCtxString "permission" env
       (consParsedCtx (Text.unpack nm) (ValuePermRepr tp) args_ctx) tp (Text.unpack p_str)

     -- Generate the type description for the body of the recursive perm
     d_tp <- tpDescTypeM sc
     let d_ident = mkSafeIdent mnm (Text.unpack nm ++ "__desc")
     ...
     let transf_ident = mkSafeIdent mnm (Text.unpack nm)
     ...
     env' <-
       permEnvAddRecPermM env (Text.unpack nm) args tp transf_ident d_ident mb_p
       NameNonReachConstr
...
heapster_define_reachability_perm _bic _opts henv nm args_str tp_str p_str trans_fun_str =
  do env <- liftIO $ readIORef $ heapsterEnvPermEnvRef henv
     ...
     Some (tp :: TypeRepr tp) <- parseTypeString "permission type" env (Text.unpack tp_str)
     (Some pre_args_ctx,
      last_args_ctx :: ParsedCtx (RNil :> tp)) <-
       do some_args_ctx <- parseParsedCtxString "argument types" env (Text.unpack args_str)
       ...
       (\npn tmp_env ->
           ...
           do trans_fun_tp <-
                ...
                (nus (cruCtxProxies args :>: Proxy) $ \(ns :>: _ :>: z) ->
                  ValPerm_Named npn (namesToExprs (ns :>: z)) NoPermOffset)
              trans_ident <-
                parseAndInsDef henv ("trans_" ++ Text.unpack nm) trans_fun_tp (Text.unpack trans_fun_str)
...
heapster_define_recursive_shape _bic _opts henv nm w_int args_str body_str =
  do env <- liftIO $ readIORef $ heapsterEnvPermEnvRef henv
     ...
     mb_sh <- parseExprInCtxString env (LLVMShapeRepr w)
       (consParsedCtx (Text.unpack nm) (LLVMShapeRepr w) args_ctx) (Text.unpack body_str)

     -- Add the shape to the current environment
     env' <- addRecNamedShape henv (Text.unpack nm) args w mb_sh
     liftIO $ writeIORef (heapsterEnvPermEnvRef henv) env'
...
heapster_define_perm _bic _opts henv nm args_str tp_str perm_string =
  do env <- liftIO $ readIORef $ heapsterEnvPermEnvRef henv
     ...
     perm <- parsePermInCtxString "permission body" env
                                   args_ctx tp_perm (Text.unpack perm_string)
     let env' = permEnvAddDefinedPerm env (Text.unpack nm) args tp_perm perm
     liftIO $ writeIORef (heapsterEnvPermEnvRef henv) env'
...
heapster_define_llvmshape _bic _opts henv nm w_int args_str sh_str =
  do env <- liftIO $ readIORef $ heapsterEnvPermEnvRef henv
     ...
     Some args_ctx <- parseParsedCtxString "argument types" env (Text.unpack args_str)
     let args = parsedCtxCtx args_ctx
     mb_sh <- parseExprInCtxString env (LLVMShapeRepr w) args_ctx (Text.unpack sh_str)
     let env' = withKnownNat w $ permEnvAddDefinedShape env (Text.unpack nm) args mb_sh
     liftIO $ writeIORef (heapsterEnvPermEnvRef henv) env'
...
heapster_define_opaque_llvmshape _bic _opts henv nm w_int args_str len_str tp_str d_str =
  do env <- liftIO $ readIORef $ heapsterEnvPermEnvRef henv
     ...
     Some args_ctx <- parseParsedCtxString "argument types" env (Text.unpack args_str)
     let args = parsedCtxCtx args_ctx
     mb_len <- parseExprInCtxString env (BVRepr w) args_ctx (Text.unpack len_str)
     sc <- getSharedContext
     d_tp <- tpDescTypeM sc
     d_id <- parseAndInsDef henv (Text.unpack nm ++ "__desc") d_tp (Text.unpack d_str)
     tp_tp <- liftIO $ translateExprTypeFunType sc env args
     tp_id <- parseAndInsDef henv (Text.unpack nm) tp_tp (Text.unpack tp_str)
     liftIO $ checkTypeAgreesWithDesc sc env nm tp_id args d_id
     let env' =
           withKnownNat w $ permEnvAddOpaqueShape env (Text.unpack nm) args mb_len tp_id d_id
     liftIO $ writeIORef (heapsterEnvPermEnvRef henv) env'
...
heapster_define_rust_type_qual_opt _bic _opts henv maybe_crate str =
  ...
  do Some lm <- failOnNothing ("No LLVM modules found")
     ...
     withKnownNat w $ withLeqProof leq_proof $
       do partialShape <- parseRustTypeString env w (Text.unpack str)
          case partialShape of
            NonRecShape nm ctx sh ->
              do let nsh = NamedShape { namedShapeName = Text.unpack $ crated_nm (Text.pack nm)
                                      , namedShapeArgs = ctx
            ...
            RecShape nm ctx mb_sh ->
              do let nm' = crated_nm (Text.pack nm)
                 env' <- addRecNamedShape henv (Text.unpack nm') ctx w mb_sh
                 liftIO $ writeIORef (heapsterEnvPermEnvRef henv) env'
...
heapster_add_block_hints :: HeapsterEnv -> Text -> [Int] ->
...
heapster_add_block_hints henv nm blks hintF =
  do env <- liftIO $ readIORef $ heapsterEnvPermEnvRef henv
     ...
     blkIDs <- case blks of
       ...
       _ -> forM blks $ \blk ->
         failOnNothing ("Block ID " <> Text.pack (show blk) <>
...
heapster_block_entry_hint _bic _opts henv nm blk top_args_str ghosts_str perms_str =
  do env <- liftIO $ readIORef $ heapsterEnvPermEnvRef henv
     Some top_args_p <-
       parseParsedCtxString "top-level argument context" env (Text.unpack top_args_str)
     Some ghosts_p <-
       parseParsedCtxString "ghost argument context" env (Text.unpack ghosts_str)
...
-- XXX why does this take the BuiltinContext and Options if it doesn't use them?
heapster_find_symbols :: BuiltinContext -> Options -> HeapsterEnv -> Text ->
                         TopLevel [Text]
heapster_find_symbols _bic _opts henv txt = do
...
-- one such symbol
heapster_find_symbol :: BuiltinContext -> Options -> HeapsterEnv -> Text ->
                        TopLevel Text
heapster_find_symbol bic opts henv txt =
  heapster_find_symbols bic opts henv txt >>= \syms ->
  case syms of
    [sym] -> return sym
    [] -> fail ("No symbol found matching string: " ++ T.unpack txt)
    _ -> fail ("Found multiple symbols matching string " ++ T.unpack txt ++ ": " ++
...
heapster_find_symbols_with_type :: BuiltinContext -> Options -> HeapsterEnv ->
                                   Text -> Text -> TopLevel [Text]
heapster_find_symbols_with_type _bic _opts henv str tp_str =
  let str' = T.unpack str
      tp_str' = T.unpack tp_str
  in
      ...
      concatMap (\(Some lm) ->
                  mapMaybe (\decl ->
                             if isInfixOf str' (symString $ L.decName decl) &&
                                decFunType decl == tp
                             then Just (T.pack $ symString $ L.decName decl) else Nothing)
                  (L.modDeclares $ modAST lm)
                  ++
                  mapMaybe (\defn ->
                             if isInfixOf str' (symString $ L.defName defn) &&
                                defFunType defn == tp
                             then Just (T.pack $ symString $ L.defName defn) else Nothing)
...
heapster_find_trait_method_symbol :: BuiltinContext -> Options ->
                                     HeapsterEnv -> Text -> TopLevel Text
heapster_find_trait_method_symbol bic opts henv str = do
  ...
  if not (T.isPrefixOf "<" instType && T.isSuffixOf ">" instType) then
    ...
    let unbracketedType :: Text = T.drop 1 $ T.dropEnd 1 instType
    -- split the method off the "trait::method" (may be more than one "::")
...
heapster_assume_fun_rename _bic _opts henv nm nm_to perms_string term_string =
  do let nm_to' = T.unpack nm_to
         perms_string' = T.unpack perms_string
         term_string' = T.unpack term_string
     Some lm <- failOnNothing ("Could not find symbol: " <> nm)
     ...
     withKnownNat w $ withLeqProof leq_proof $ do
        ...
        let env'' = permEnvAddGlobalSymFun env'
                                           (GlobalSymbol $ fromString $ T.unpack nm)
                                           w
                                           fun_perm
...
insPrimitive :: HeapsterEnv -> Text -> Term -> TopLevel Ident
insPrimitive henv nm tp =
  do sc <- getSharedContext
     let mnm = heapsterEnvSAWModule henv
     let ident = mkSafeIdent mnm (Text.unpack nm)
...
heapster_assume_fun_rename_prim _bic _opts henv nm nm_to perms_string =
  do Some lm <- failOnNothing ("Could not find symbol: " <> nm)
     ...
     withKnownNat w $ withLeqProof leq_proof $ do
        SomeFunPerm fun_perm <-
          parseFunPermStringMaybeRust "permissions" w env args ret (Text.unpack perms_string)
        env' <- liftIO $ readIORef (heapsterEnvPermEnvRef henv)
        fun_typ <- liftIO $ translateCompleteFunPerm sc env fun_perm
        term_ident <- insPrimitive henv nm_to fun_typ
        let env'' = permEnvAddGlobalSymFun env'
                                           (GlobalSymbol $ fromString $ Text.unpack nm)
                                           w
                                           fun_perm
                                           (globalOpenTerm term_ident)
        liftIO $ writeIORef (heapsterEnvPermEnvRef henv) env''
  ...
  do Some lm <- failOnNothing ("Could not find symbol: " <> nm)
     ...
     perms_terms :: [(SomeFunPerm (CtxToRList cargs) ret, OpenTerm)] <-
       ...
       do some_fun_perm <-
          ...
          term_ident <-
            parseAndInsDef henv (Text.unpack nm ++ "__" ++ show i) fun_typ (Text.unpack term_string)
          return (some_fun_perm, globalOpenTerm term_ident)
     let env' =
           withKnownNat w $ withLeqProof leq_proof $
           permEnvAddGlobalSymFunMulti env (GlobalSymbol $
                                            fromString $ Text.unpack nm) w perms_terms
     liftIO $ writeIORef (heapsterEnvPermEnvRef henv) env'
...
-- name and a 'Text' representation of its permission
...
heapster_typecheck_mut_funs_rename _bic opts henv fn_names_and_perms =
  do let fst_nm =
     ...
     some_cfgs_and_perms <- forM fn_names_and_perms $ \(nm, nm_to, perms_string) ->
       do let nm' = Text.unpack nm
              nm_to' = Text.unpack nm_to
          AnyCFG cfg <-
            failOnNothing ("Could not find symbol definition: " <> nm) =<<
...
heapster_set_event_type _bic _opts henv term_string =
  do sc <- getSharedContext
     ...
     ev_id <- parseAndInsDef henv "HeapsterEv" ev_tp (Text.unpack term_string)
...
heapsterFunTrans :: HeapsterEnv -> Text -> TopLevel Term
heapsterFunTrans henv fn_name =
  do sc <- getSharedContext
     let saw_modname = heapsterEnvSAWModule henv
     fun_term <-
       fmap (fromJust . defBody) $
       liftIO $ scRequireDef sc $ mkSafeIdent saw_modname (Text.unpack fn_name)
     bodies <-
       liftIO $ scResolveName sc $ fn_name <> "__bodies"
...
heapster_export_coq :: HeapsterEnv -> FilePath -> TopLevel ()
heapster_export_coq henv filename =
  ...
  do env <- liftIO $ readIORef $ heapsterEnvPermEnvRef henv
     ...
     withKnownNat w64 $ withLeqProof leq_proof $ do
        Some3FunPerm fun_perm <-
          parseSome3FunPermFromRust env w64 (Text.unpack perms_string)
...
heapster_parse_test _bic _opts _some_lm@(Some lm) fn_name perms_string =
  do let env = heapster_default_env -- FIXME: env should be an argument
     ...
     SomeFunPerm fun_perm <- parseFunPermString "permissions" env args
                                                ret (Text.unpack perms_string)
     liftIO $ putStrLn $ permPrettyString emptyPPInfo fun_perm

-- | Dump the IDE information contained in a Heapster environment to a JSON file
heapster_dump_ide_info :: HeapsterEnv -> FilePath -> TopLevel ()
heapster_dump_ide_info henv filename = do
