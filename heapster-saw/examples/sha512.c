...
uint64_t simpl0(const uint8_t *in, size_t num) {
  uint64_t sum = 0;
...
uint64_t SIMPL1_CRYPTO_load_u64_be(const void *ptr) {
  uint64_t ret;
  memcpy(&ret, ptr, sizeof(ret));
  return ret;
}

#define SIMPL1_ROUND_00_01(i, a) T1 += a;

#define SIMPL1_ROUND_02_10(i, j, a) T1 += a;

void sha512_block_data_order_simpl1(uint64_t *state, const uint8_t *in, size_t num) {
  uint64_t a, T1;
  int i;
  ...
  while (num--) {
    ...
    SIMPL1_ROUND_00_01(0, a);
    T1 = SIMPL1_CRYPTO_load_u64_be(in + 8);
    SIMPL1_ROUND_00_01(1, a);

    for (i = 2; i < 10; i += 2) {
      SIMPL1_ROUND_02_10(i, 0, a);
      SIMPL1_ROUND_02_10(i, 1, a);
    }
