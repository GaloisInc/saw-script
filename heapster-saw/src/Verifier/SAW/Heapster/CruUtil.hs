...
-- | Prove that converting the 'RAssign' back to an 'Assignment' would have the
-- same type
...
assignToRListWithEq :: Assignment f ctx -> (RAssign f (CtxToRList ctx),
                                            RListToCtx (CtxToRList ctx) :~: ctx)
assignToRListWithEq asgn = case viewAssign asgn of
  AssignEmpty -> (MNil, Refl)
  AssignExtend asgn' f
    | (fs, Refl) <- assignToRListWithEq asgn' -> (fs :>: f, Refl)
...
-- | Prove that converting an 'RAssign' to an 'Assignment' and back yields an
-- equal type to that of the original 'RAssign'
...
-- | Build a 'CruCtx' from a 'CtxRepr' and return a proof that converting the
-- 'CruCtx' back to an 'CtxRepr' would have the same type
mkCruCtxWithEq :: CtxRepr ctx -> (CruCtx (CtxToRList ctx),
                                  RListToCtx (CtxToRList ctx) :~: ctx)
mkCruCtxWithEq ctx = case viewAssign ctx of
  AssignEmpty -> (CruCtxNil, Refl)
  AssignExtend ctx' tp
    | (tps, Refl) <- mkCruCtxWithEq ctx' -> (CruCtxCons tps tp, Refl)
