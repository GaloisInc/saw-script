...
bindTParam' sc tp env =
  ...
  let env' = addTyVars (Map.singleton (C.tpUnique tp) v) env
  return (env', v, k)
...
bindName sc name schema env = do
  ...
  let env' = addAllTerms (Map.singleton name v) $
             addAllVars (Map.singleton name schema)
             env
...
bindProp sc prop nm env =
  ...
  let env' = addTyProps (insertSupers prop [] v mempty) env
  return (env', v)
...
importDeclGroup declOpts sc env0 (C.Recursive decls) =
  do let binds = [ (C.dName d, C.dSignature d) | d <- decls ]
     ...
     pure $ addAllTerms rhss $
            addAllVars (Map.fromList binds)
            env0
...
importDeclGroup declOpts sc env (C.NonRecursive decl) = do
  ...
  pure $ addAllTerms (Map.singleton (C.dName decl) rhs) $
         addAllVars (Map.singleton (C.dName decl) (C.dSignature decl))
         env
...
translateType sc env ty = importType sc env ty

translateSchema :: SharedContext -> CryptolEnv -> C.Schema -> IO Term
translateSchema sc env ty = importSchema sc env ty

translateExpr :: SharedContext -> CryptolEnv -> C.Expr -> IO Term
translateExpr sc env expr = importExpr sc env expr
...
translateDeclGroups sc env1 dgs =
  do
     ...
     pure $ addExtraVars newVars env3
...
genCodeForNominalTypes sc nominalMap env0 =
  ...
  where
    ...
    updateEnvForNominal env nt = do
      ...
      return $ addAllTerms (Map.fromList constrs) $
               addAllVars (Map.fromList conTs) env
