...
import SAWCore.Module (ModuleMap, Def(..), ResolvedName(..), requireNameInMap, resolvedNameType)
import SAWCore.Name
...
asTypedGlobalDef :: (?mm :: ModuleMap) => Recognizer Term GlobalDef
asTypedGlobalDef t =
...
monadifyArg :: HasCallStack => (?mm :: ModuleMap) => Maybe MonType -> Term -> MonadifyM ArgMonTerm
{-
monadifyArg _ t
...
monadifyArgTerm ::
  HasCallStack => (?mm :: ModuleMap) =>
  Maybe MonType -> Term -> MonadifyM OpenTerm
monadifyArgTerm mtp t = usingEvType (toArgTerm <$> monadifyArg mtp t)

-- | Monadify a term
monadifyTerm :: (?mm :: ModuleMap) => Maybe MonType -> Term -> MonadifyM MonTerm
{-
monadifyTerm _ t
...
monadifyTerm' :: HasCallStack => HasSpecMEvType => (?mm :: ModuleMap) =>
...
monadifyApply :: HasCallStack => (?mm :: ModuleMap) => MonTerm -> [Term] -> MonadifyM MonTerm
...
-- branch
iteMacro :: (?mm :: ModuleMap) => MonMacro
...
-- to the body
invariantHintMacro :: (?mm :: ModuleMap) => MonMacro
...
-- body to the body
assertingOrAssumingMacro :: (?mm :: ModuleMap) => Bool -> MonMacro
...
fixMacro :: (?mm :: ModuleMap) => MonMacro
...
defaultMonEnv :: (?mm :: ModuleMap) => MonadifyEnv
...
defaultMonTable :: (?mm :: ModuleMap) => Map NameInfo MonMacro
defaultMonTable =
  Map.fromList
  [
    -- Prelude functions
...
monadifyTermInEnvH sc top_trm top_tp =
  do lift $ ensureCryptolMLoaded sc
     ...
     forM_ const_infos $ \(vi, nmi) ->
       do let r = requireNameInMap (Name vi nmi) mm
          let tp = resolvedNameType r
          let maybe_body =
                case r of
