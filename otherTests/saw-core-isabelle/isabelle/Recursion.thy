...
cryptol_definition rec_fnlba_gt_0rb_impl :: "{'a,'b} ((fin 'a,Ring 'b,Zero 'b,'a > 0,Eq 'b) =?> ((['a]'b) \<Rightarrow> 'b))"

(* Recursive call stub (to be overridden)*)
cryptol_definition rec_fn_impl :: "{'a,'b} ((fin 'a,Ring 'b,Zero 'b,Eq 'b) =?> ((['a]'b) \<Rightarrow> 'b))"

(* Recursive call schema*)
cryptol_definition rec_fnlba_gt_0rb_spec :: "{'a,'b} ((fin 'a,Ring 'b,Zero 'b,'a > 0,Eq 'b) =?> ((['a]'b) \<Rightarrow> 'b))" where
"rec_fnlba_gt_0rb_spec x \<equiv> (head`{'a - 1,'b} x) +`{'b} (rec_fn_impl`{'a - 1,'b} (tail`{'a - 1,'b} x))"

cryptol_definition rec_fnlba_eqeq_0rb :: "{'a,'b} ((fin 'a,Ring 'b,Zero 'b,'a = 0,Eq 'b) =?> ((['a]'b) \<Rightarrow> 'b))" where
"rec_fnlba_eqeq_0rb x \<equiv> zero`{'b}"

(* Recursive call schema*)
cryptol_definition rec_fn_spec :: "{'a,'b} ((fin 'a,Ring 'b,Zero 'b,Eq 'b) =?> ((['a]'b) \<Rightarrow> 'b))" where
"rec_fn_spec  \<equiv> 
  let
