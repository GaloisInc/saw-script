...
/-
SLICE R2 DISCHARGE - popcount recurrence (4-bit input, width-3
accumulator, N = 5 chain).
...
noncomputable def pcBody (bits : Except String (Vec 4 Bool)) :
    Except String (Vec 5 (Vec 3 Bool)) →
      Except String (Vec 5 (Vec 3 Bool)) :=
  ...
  (CryptolToLean.SAWCorePrimitives.natPos_macro
  (CryptolToLean.SAWCorePrimitives.bit1_macro
  CryptolToLean.SAWCorePrimitives.one_macro)) Bool) UnitType)) (Bind.bind bits
  (fun v_4 => Bind.bind ic (fun v_5 => Pure.pure (zip Bool (Vec
  (CryptolToLean.SAWCorePrimitives.natPos_macro
  (CryptolToLean.SAWCorePrimitives.bit1_macro
  CryptolToLean.SAWCorePrimitives.one_macro)) Bool)
  (CryptolToLean.SAWCorePrimitives.natPos_macro
  ...
  (CryptolToLean.SAWCorePrimitives.natPos_macro
  (CryptolToLean.SAWCorePrimitives.bit1_macro
  CryptolToLean.SAWCorePrimitives.one_macro)) Bool) UnitType) v_2))) (Bind.bind
  x__' (fun v_1 => Bind.bind (Pure.pure (bvNat
  (CryptolToLean.SAWCorePrimitives.natPos_macro
  (CryptolToLean.SAWCorePrimitives.bit1_macro
  CryptolToLean.SAWCorePrimitives.one_macro))
  (CryptolToLean.SAWCorePrimitives.natPos_macro
...
noncomputable def pcElem (bits : Vec 4 Bool) (v : Vec 5 (Vec 3 Bool))
    (i : Nat) (hi : i < 5) : Vec 3 Bool :=
  if h : i < 1 then bvNat 3 0
  else
    (bif bits[i - 1]'(by omega) then
      bvAdd 3 (v[i - 1]'(by omega)) (bvNat 3 1)
    else v[i - 1]'(by omega))

/-- The closed-form step: what one pcBody application computes on a
pure input. -/
noncomputable def pcStep (bits : Vec 4 Bool) (v : Vec 5 (Vec 3 Bool)) :
    Vec 5 (Vec 3 Bool) :=
  Vector.ofFn (fun i : Fin 5 => pcElem bits v i.val i.isLt)

theorem pcBody_pure_eq (bits : Vec 4 Bool) (v : Vec 5 (Vec 3 Bool)) :
  ...
  · have hcond : ltNat i (natPos_macro one_macro) = false := by
    ...
    refine Eq.trans (atWithProof_gen_ok (n := 4) _
      (fun j hj =>
        bif bits[j]'(by omega) then
          bvAdd 3 (v[j]'(by omega)) (bvNat 3 1)
        else v[j]'(by omega))
    ...
    · intro j hj
      ...
      -- lemma's collection length is `Nat.min 4 5`, the goal's is
      -- the (reducible) literal 4, and simp/rw matching does not
      ...
      have hz : @GetElem.getElem
          (Vec 4 (PairType Bool (PairType (Vec 3 Bool) UnitType)))
          Nat _ _ _
          (zip Bool (Vec 3 Bool) 4 5 bits v) j hj
          = PairType.PairValue (bits[j]'hj)
              (PairType.PairValue (v[j]'(by omega)) UnitType.Unit) :=
        zip_getElem_lt 4 5 bits v j hj
      rw [hz]
    · simp [pcElem, h1]
...
running_sum's. -/
theorem pc_h_prod (bits : Vec 4 Bool) :
    saw_fix_bounded_productive 5 (Vec 3 Bool) (pcBody (Pure.pure bits)) := by
  refine ⟨⟨Vector.replicate 5 (bvNat 3 0)⟩, ?_, ?_⟩
  · intro v
...
noncomputable def pcChain (bits : Vec 4 Bool) : Nat → Vec 3 Bool
  | 0 => bvNat 3 0
  | k + 1 =>
    if h : k < 4 then
      (bif bits[k]'h then bvAdd 3 (pcChain bits k) (bvNat 3 1)
       else pcChain bits k)
    else pcChain bits k

/-- The closed-form solution vector. -/
noncomputable def pcSol (bits : Vec 4 Bool) : Vec 5 (Vec 3 Bool) :=
  Vector.ofFn (fun i : Fin 5 => pcChain bits i.val)

/-- The closed form is a fixed point of the body. -/
theorem pc_sol_fixed (bits : Vec 4 Bool) :
  ...
  -- at N = 5 the running_sum-style `congr 1` defeq close hits the
...
theorem pc_choose_eq (bits : Vec 4 Bool)
    (H : saw_fix_bounded_productive 5 (Vec 3 Bool)
      (pcBody (Pure.pure bits))) :
    saw_fix_bounded_choose 5 (Vec 3 Bool) (pcBody (Pure.pure bits)) H =
      Pure.pure (pcSol bits) :=
  (saw_fix_bounded_choose_unique_pure_fixed_point 5 (Vec 3 Bool)
...
throughout). -/
theorem pc_natRec_eq (bits : Vec 4 Bool) :
    ∀ k, k ≤ 4 →
      Nat.rec (motive := fun _ => Vec 3 Bool) (bvNat 3 0)
        (fun i acc =>
          bif atWithDefault 4 Bool false bits i then
            bvAdd 3 acc (bvNat 3 1)
          else acc) k
      = pcChain bits k := by
  intro k
  induction k with
  ...
  | succ m ih =>
    ...
    rw [← ih (by omega)]
    rw [← atWithDefault_lt (n := 4) false bits m hm]
...
noncomputable def goal : Prop :=
  ...
  (CryptolToLean.SAWCorePrimitives.bit0_macro
  (CryptolToLean.SAWCorePrimitives.bit0_macro
  CryptolToLean.SAWCorePrimitives.one_macro))) Bool) -> let bits := (Pure.pure
  bits0); @Eq.{1} (Except String Bool) (Bind.bind (foldlM Bool (Vec
  (CryptolToLean.SAWCorePrimitives.natPos_macro
  (CryptolToLean.SAWCorePrimitives.bit1_macro
  CryptolToLean.SAWCorePrimitives.one_macro)) Bool)
  (CryptolToLean.SAWCorePrimitives.natPos_macro
  ...
  (CryptolToLean.SAWCorePrimitives.natPos_macro
  (CryptolToLean.SAWCorePrimitives.bit1_macro
  CryptolToLean.SAWCorePrimitives.one_macro)) v_1 v_2)))) acc) (Pure.pure (bvNat
  (CryptolToLean.SAWCorePrimitives.natPos_macro
  (CryptolToLean.SAWCorePrimitives.bit1_macro
  CryptolToLean.SAWCorePrimitives.one_macro))
...
theorem goal_holds : goal := by
  ...
  have hfold :
      foldlM Bool (Vec 3 Bool) 4
        (fun (acc : Except String (Vec 3 Bool)) (b : Except String Bool) =>
          CryptolToLean.SAWCorePreludeExtra.iteM (Vec 3 Bool) b
            (Bind.bind acc (fun v_1 => Bind.bind (Pure.pure (bvNat 3 1))
              (fun v_2 => Pure.pure (bvAdd 3 v_1 v_2)))) acc)
        (Pure.pure (bvNat 3 0)) (Pure.pure bits0)
      = Pure.pure (foldl Bool (Vec 3 Bool) 4
          (fun acc b => bif b then bvAdd 3 acc (bvNat 3 1) else acc)
          (bvNat 3 0) bits0) :=
    foldlM_pure_eq_foldl _ _ _ _ _ _ _ (fun acc a => by cases a <;> rfl)
  rw [hfold]
  have hchain : foldl Bool (Vec 3 Bool) 4
      (fun acc b => bif b then bvAdd 3 acc (bvNat 3 1) else acc)
      (bvNat 3 0) bits0 = PCDischarge.pcChain bits0 4 := by
    rw [foldl_eq_natRec_atWithDefault Bool (Vec 3 Bool) 4 _ _ _ false]
    exact PCDischarge.pc_natRec_eq bits0 4 (by omega)
  rw [hchain]
