/-
IN-ITP DECOMPOSITION PILOT — discharge of the s20_rowround obligation.

Source: workflows/llvm_rowround_itp/test_llvm_rowround_itp.saw, which
verifies s20_rowround with NO overrides. Symbolic execution inlines the
four C quarterround calls into ONE goal over a single [16][32] state;
the emitted RHS is `rowround y` (the Cryptol spec). SAW never learns
...
fallbacks (index-safety obligations). Those fallbacks are dead
— the `omega`/`simp;omega` branch closes every bound first — so this file
is the generated outline with those never-taken fallbacks stripped and
`goal_holds` discharged. The harness drift-checks that the stripped
`goal` is definitionally the generated `goal`.

The discharge, in three parts:

  1. THE IN-ITP OVERRIDE (`rotl_shlor_32`). Both the inlined C words and
     the Cryptol spec words are the SAME quarterround arithmetic; they
     differ ONLY in how a 32-bit left-rotate is written — the C compiler
     lowers `<<<` to a shift-or `(x << k) | (x >> (32-k))`, while Cryptol
     keeps `rotateL`. `rotl_shlor_32` proves these equal via the checked
     `BitVec.rotateLeft_def` bridge (NO bv_decide). This lemma is the
     kernel-checked stand-in for the SAW quarterround override, applied
     at all 16 rotate sites (`rotl_7/9/13/18`). After it fires, the two
     sides are spec-vs-spec (rotateL vs rotateL) — the BV wall that
     blocks the standalone quarterround proof never appears here.

  2. MONADIC SCAFFOLDING REDUCTION. The emitted goal is a deeply-nested
     eager `Except`-monad fold checking the two [16][32] states equal
     element-wise. `seq4`/`seq16` (abstract literal-sequence lemmas,
     proved once over opaque elements) collapse the `vecSequenceM`
     literals in a single rewrite each — cheap, no inline peeling — and
     the standard monad/`ofFnM_except_ok`/`foldrM` rewrites reduce the
     whole thing to a pure `Vector.foldr` over per-word `bvEq`s.

  3. PER-WORD CLOSE. The C and spec words still differ by XOR argument
     order (`rot ^ y` vs `y ^ rot`, at every node). `foldr_ofFn_all_true`
     turns the fold into 16 per-word `bvEq` goals; each is pushed to
     `BitVec` (directed bridges) and closed by `ac_rfl` (XOR is AC).
     Doing this per-word keeps each step bounded — a single `bvXor_comm`
     over the whole 16-word state is intractable.
...
noncomputable def goal : Prop :=
  (y : Vec (CryptolToLean.SAWCorePrimitives.natPos_macro
  ...
  (CryptolToLean.SAWCorePrimitives.bit0_macro
  (CryptolToLean.SAWCorePrimitives.bit0_macro
  CryptolToLean.SAWCorePrimitives.one_macro)))))) Bool)) -> let y := (Pure.pure
  y); @Eq.{1} (Except String Bool) (foldrM Bool Bool
  (CryptolToLean.SAWCorePrimitives.natPos_macro
  (CryptolToLean.SAWCorePrimitives.bit0_macro
  (CryptolToLean.SAWCorePrimitives.bit0_macro
  (CryptolToLean.SAWCorePrimitives.bit0_macro
  ...
  (CryptolToLean.SAWCorePrimitives.bit0_macro
  (CryptolToLean.SAWCorePrimitives.bit0_macro
  CryptolToLean.SAWCorePrimitives.one_macro)))))) Bool) y
  ...
  (Bind.bind (Bind.bind x__''''''''''''''''''''' (fun v_1 => Pure.pure (bvShl
  (CryptolToLean.SAWCorePrimitives.natPos_macro
  (CryptolToLean.SAWCorePrimitives.bit0_macro
  (CryptolToLean.SAWCorePrimitives.bit0_macro
  (CryptolToLean.SAWCorePrimitives.bit0_macro
