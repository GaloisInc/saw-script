/-
ChaCha20 quarterround verify — completed outline, ACCEPTED under the
`native-eval` trust tier (2026-07-21 user decision; see .trust-tier).

The SAW driver `llvm_chacha20_q_verify` calls the unmodified reference
`qround` from `examples/chacha20/chacha20.c` on a symbolic 16-word
state, then asserts via `llvm_points_to` that the post-call state
equals the Cryptol-spec `qround` output spliced into the input state
at column positions (0, 4, 8, 12). The Cryptol spec is the unmodified
`deps/cryptol-specs/.../chacha20.cry`.

Completed-outline row: the generated `def goal` embeds vacuous
bounds-proof fallbacks whose dead admit-placeholder tails are stripped
here (proof irrelevance keeps `goal` rfl-equal to the generated goal;
the harness drift check enforces that), and `goal_holds` is
discharged. Scaffold mirrors proofs/llvm_rowround_itp: seq/atWithProof/
genWithBounds normalization + the foldr_ofFn_all_true bridge, with the
rotate-granularity bridge at the ChaCha20 rotation amounts
{16, 12, 8, 7}. The 12 unchanged state positions close by bvEq_refl;
the 4 qround positions are genuine LLVM-vs-spec 32-bit equations
closed by `bv_decide`.
...
pass with no admitted placeholders), swap `bv_decide` -> `smt` here and delete
.trust-tier, migrating this row to the strict tier with NO other
changes — the proof structure is already tactic-swap-ready.
-/
...
noncomputable def goal : Prop :=
  ...
  (CryptolToLean.SAWCorePrimitives.bit0_macro
  CryptolToLean.SAWCorePrimitives.one_macro)))))) v_1'
  v_2))), x__'''''''''''''''', x__''''''''''''''''', x__'''''''''''''''''', x__'''''''''''''', x__''''''''''''''''''', x__'''''''''''''''''''', x__''''''''''''''''''''', x__''''''''''''', x__'''''''''''''''''''''', x__''''''''''''''''''''''', x__''''''''''''''''''''''''])
...
theorem goal_holds : goal := by
  ...
  -- Positions 0, 4, 8, 12 carry the qround equations (LLVM-vs-spec,
  -- closed by bv_decide under the native-eval trust tier); the other
  -- 12 positions collapsed to `true = true` via bvEq_refl in the
  -- scaffold pass.
