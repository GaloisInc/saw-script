...
is_waived() {
    case "$1" in
        ...
        # axiom-or-macro-decl-in-user-file IS pinned end-to-end
        # through SAW (saw-boundary/replay_reject_axiom,
        # _suffix_axiom, and replay_reject_notation), which is a
        # STRONGER pin than a kernel-level stage.
        #
        # axiom-outside-allowlist is NOT: corrected 2026-07-29 by the
        # session audit, which found none of the cited rows ever
        # reaches the axiom audit — all three exit earlier, at the
        # source lint or the sorry scan, with a DIFFERENT diagnostic.
        # The waiver claimed a mechanism that does not exist, inside
        # the very meta-guard whose job is to stop guards going
        # unwatched. It is waived here only because the guard IS
        # exercised (green) by the kernel selftest's own coercion
        # case, which reaches step 7 and reports
        # "OK[coercion]: rejected with 'axiom-outside-allowlist'";
        # the OWED work is a saw-boundary row carrying that vector
        # through the runtime path, tracked in TODO.md's pins-owed
        # section.
